...
func (r *Racer) RunH2Race(ctx context.Context, reqSpec *models.CapturedRequest, concurrency int) ([]models.ScanResult, error) {
	...
	// Performance optimization: Create a template request once to avoid
	// repeated URL parsing and header map allocation in the loop.
	baseReq, _ := http.NewRequestWithContext(raceCtx, method, reqSpec.URL, nil)
	baseReq.Header.Set("User-Agent", "Scalpel-Racer/Go-H2")
	...
	for i := 0; i < concurrency; i++ {
		...
		// Clone the base request (deep copies URL and Header)
		req := baseReq.Clone(raceCtx)
		// Set the body reader freshly for each request
		req.Body = io.NopCloser(bytes.NewReader(reqSpec.Body))
		// Add unique ID
