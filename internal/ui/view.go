	...
	case StateIntercepting:
		if m.History.size == 0 {
			...
			msg := lipgloss.JoinVertical(lipgloss.Center,
				...
				emptySubtitleStyle.Render(fmt.Sprintf("Listening on :%d", m.ProxyPort)),
				emptySubtitleStyle.Render(fmt.Sprintf("Configure your client to proxy: http://127.0.0.1:%d", m.ProxyPort)),
			)
