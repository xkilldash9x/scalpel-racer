...
def format_request_display(req) -> str:
    ...
    payload = req.get_attack_payload() if hasattr(req, 'get_attack_payload') else (req.body if hasattr(req, 'body') else b"")
    ...
    m = req.method.upper()
    if m == 'GET': method_color = Fore.GREEN
    elif m == 'POST': method_color = Fore.CYAN
    elif m == 'DELETE': method_color = Fore.RED
    elif m in ('PUT', 'PATCH'): method_color = Fore.YELLOW
    else: method_color = Fore.WHITE
...
def analyze_results(results: List[ScanResult]):
    ...
    if not signatures: 
        ...
        for key in sorted_keys:
            ...
            print_formatted_text(ANSI(f"  {len(group):<6} {sc_color}{status_code:<6}{Style.RESET_ALL} {hash_short:<16} {snippet}"))
        print("  " + "-" * 90)
...
class ScalpelApp:
    def __init__(self, port, strategy):
        ...
        self.session = PromptSession(
            ...
            key_bindings=self._create_key_bindings() if UI_AVAILABLE else None
        )
        self.mgr = None
        self.cert_mgr = None
