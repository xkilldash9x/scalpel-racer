...
# Removed global pytestmark to prevent warnings on sync tests
...
async def test_send_probe_advanced_differential_analysis():
    ...
    # Check if the content is an async generator (the specific type check is complex due to async context)
    assert hasattr(kwargs['content'], '__aiter__')
    
    # Verify default Content-Type was added (updated default value)
...
async def test_run_scan_staged_strategy_setup():
    ...
    # Mock dependencies
    with patch("scalpel_racer.httpx.AsyncClient"), \
