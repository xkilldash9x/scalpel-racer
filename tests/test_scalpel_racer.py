...
"""
Integration tests for the main Scalpel Racer application.
"""
import os
import asyncio
...
class TestScalpelIntegration:
    """Integration tests for Scalpel Racer components."""
    ...
    async def test_run_scan_standard(self, mock_client):
        ...
        with patch("gc.disable"), patch("gc.enable"):
            results = await run_scan(
                req, concurrency=2, http2=False, warmup=0, strategy="auto"
            )
    ...
    async def test_run_scan_h2_delegation(self, mock_h2_cls):
        ...
        with patch("gc.disable"), patch("gc.enable"):
            results = await run_scan(
                req, concurrency=1, http2=True, warmup=0, strategy="spa"
            )
    ...
    async def test_send_probe_advanced_usage(self):
        ...
        # [FIX] Pass mocked base_headers
        mock_headers = MagicMock()
        mock_headers.copy.return_value = {}

        # Case 1: Standard send (No barriers)
        await send_probe_advanced(
            mock_client, req, payload, barriers=[], warmup_ms=0, index=0,
            is_staged=False, base_headers=mock_headers
        )
        ...
        await send_probe_advanced(
            mock_client, req, payload, barriers=[], warmup_ms=100, index=0,
            is_staged=False, base_headers=mock_headers
        )
    ...
    def test_ca_manager_shared_key_optimization(self):
        ...
        with patch("builtins.open", mock_open()), \
            ...
            # We already instantiated it above.
            # To test re-init or just verify calls:
            # The constructor logic runs once.
    ...
    def test_scalpel_app_handler(self, mock_session, mock_print): # pylint: disable=unused-argument
...
class TestAppIntegration:
    ...
    @pytest.mark.asyncio
    async def test_app_startup_wiring(
        self, mock_patch_stdout, mock_session, mock_pm, mock_cm, mock_print
